...
{
    if (is_empty())
    //has to return smth but cannot return. instantly throw  
        throw string("ERROR: Can't remove from an empty queue.");

    T val = data[first];
    size--;
    //or if we dont do size -- we do size ==0
    if (size == 0) {
        last = -1;
        first = -1;        
    }   
    ...
    //if size is wayyy smaller than capacity 
    // 1/4 not 1/2 cause resize in enqeue is *2
    //keep in mind that the resizing is linear but
